...
sub find_same_filename2 {
    my ($mydir, $myfilenam) = @_;
	 for (keys %LISTOBJECT) {
		  if (("$LISTOBJECT{$_}->{FILENAME}.$LISTOBJECT{$_}->{EXTENSION}" eq $myfilename) 
				and ($LISTOBJECT{$_}->{FULLPATH_SRC} ne $mydir)) {
...
sub rec_print_dependencies {
    ...
    #while(my($dep_filename, $dep_ptr) = each(%{$file->{DEPENDENCIES}})) {
	 for my $dep_filename (sort keys %{$file->{DEPENDENCIES}}) {
        ...
        $tmp_filename = "$dep_ptr->{FULLPATH_SRC}$dep_ptr->{FILENAME}.o" if ($dep_ptr->{COMPILABLE});
