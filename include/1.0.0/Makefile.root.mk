## ====================================================================
## File: $purplefrog/include/$PF_COMPILER_VERSION/Makefile.root.mk
##
...
#MAKE_LINKS  := $(MYTIME) .pfupdate_build_links.ksh

.DEFAULT:
	$(MYTIME) .pfupdate_build_links.ksh ;\
	cd $(BUILDOBJ) ;\
...
$(BUILDOBJ)/Makefile: Makefile.user.mk $(BUILDOBJ)/Makefile.local.mk $(BUILDOBJ)/Makefile.rules.mk $(BUILDOBJ)/Makefile.base_arch.mk $(BUILDOBJ)/Makefile.ec_arch.mk $(BUILDOBJ)/Makefile.dep.mk
	cp $(PURPLEFROGINC)/Makefile.build.mk $(BUILDOBJ)/ 2>/dev/null || true ;\
...
$(BUILDOBJ)/Makefile.rules.mk:
	cp $(PURPLEFROGINC)/Makefile.rules.mk $@ 2>/dev/null || true ;\
	touch $@
$(BUILDOBJ)/Makefile.base_arch.mk:
	cp $(PURPLEFROGINC)/$(BASE_ARCH)/Makefile.arch.mk $@ 2>/dev/null || true ;\
	touch $@
$(BUILDOBJ)/Makefile.ec_arch.mk:
	cp $(PURPLEFROGINC)/$(EC_ARCH)/Makefile.arch.mk $@ 2>/dev/null || true ;\
	touch $@
$(BUILDOBJ)/Makefile.dep.mk:
	cd $(BUILDSRC) ;\
	$(MYTIME) $(PERLPROF)pfdependencies.pl $(VERBOSE) --deep-include --soft-restriction $(DEP_DUP_OK) $(DEP_FLAT) --out=$(BUILDOBJ)/Makefile.dep.mk --any --short --supp=$(PFDEP_SUPP_FILE) --inc=`find * -type d -name include|tr '\n' ':'` `find * -type d|grep -v include`
